...
import {
  AddPainEntryDto,
} from './dto/add-pain-entry.dto';

import {
  AddWorkoutDto,
} from './dto/add-workout.dto';
...
} from './dto/update-health-entry.dto';

import {
  UpdateHealthHabitsDto,
} from './dto/update-health-habits.dto';

import {
  UpdatePainEntryDto,
} from './dto/update-pain-entry.dto';
...
export class HealthService {
  ...
  async create(
    ...
    const existing =
      await this.healthEntryModel.exists({
        dateKey,
      });
    ...
    const entry =
      await this.healthEntryModel.create({
        ...dto,
  ...
  async findAll(
    ...
    const filter:
      QueryFilter<HealthEntryDocument> = {
      isActive: true,
    };
  ...
  async findByDate(
    ...
    if (!entry) {
      throw new NotFoundException(
        `Health entry not found for ${dateKey}.`,
      );
    }
  ...
  async resolvePainEntry(
    healthEntryId: string,
    painIndex: number,
  ) {
    return this.updatePainEntry(
      healthEntryId,
      painIndex,
      {
        resolved: true,
  ...
  private async setArchiveStatus(
    healthEntryId: string,
    isArchived: boolean,
  ) {
    this.validateObjectId(
