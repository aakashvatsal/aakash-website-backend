...
export class CompaniesService {
  ...
  async create(dto: CreateCompanyDto) {
    ...
    const existing = await this.companyModel.exists({ slug, isActive: true });
    if (existing) {
  ...
  async findAll(query: CompanyQueryDto) {
    ...
    const filter: QueryFilter<Company> = { isActive: true };
  ...
  async findPublic(query: CompanyQueryDto) {
    const result = await this.findAll({ ...query, isArchived: false });
    return {
      ...result,
  ...
  async findOne(companyId: string) {
    ...
    const company = await this.companyModel
      .findOne({ _id: new Types.ObjectId(companyId), isActive: true })
      .lean();
    if (!company) throw new NotFoundException('Company not found.');
    return company;
  }
  ...
  async findBySlug(slug: string) {
    const company = await this.companyModel
      .findOne({ slug: this.generateSlug(slug), isActive: true })
      .lean();
    if (!company) throw new NotFoundException('Company not found.');
    return company;
  }
  ...
  async addMetric(companyId: string, dto: AddCompanyMetricDto) {
    ...
    if (metricIndex >= 0) company.metrics[metricIndex] = metric;
    else company.metrics.push(metric);
    await company.save();
    return company;
  }
  ...
  async updateGoal(
    ...
    if (dto.title !== undefined) currentGoal.title = dto.title;
  ...
  async updatePriorities(companyId: string, priorities: string[]) {
    const updated = await this.companyModel
      .findOneAndUpdate(
        { _id: new Types.ObjectId(companyId), isActive: true },
        { $set: { currentPriorities: priorities, lastReviewedAt: new Date() } },
        { new: true, runValidators: true },
      )
      .lean();
