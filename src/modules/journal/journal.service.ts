...
export class JournalService {
  ...
  async update(
    ...
    if (
      ...
      journalEntry.productivityScore =
        dto.productivityScore;
    }
    ...
    if (
      ...
      journalEntry.isPublished =
        dto.isPublished;

      if (
        dto.isPublished
      ) {
        journalEntry.publishedAt =
          dto.publishedAt
            ? this.parseDate(
                dto.publishedAt,
              )
  ...
  async restore(
    ...
    const journalEntry =
      await this.journalModel.findById(
        this.toObjectId(
          journalEntryId,
        ),
      );
  ...
  private async slugExists(
    ...
    const exists =
      await this.journalModel.exists(
        filter,
      );
  ...
  private toDateKey(
    ...
    if (
      ...
      throw new BadRequestException(
        'Unable to generate journal date key.',
      );
    }
